...
async def create_cantine_menu_item(item: CantineMenuItem, db=Depends(get_db)):
    cantine_menu_db = CantineMenuDB(db)
...
async def update_cantine_menu_item(item_id: str, item: CantineMenuItem, db=Depends(get_db)):
    cantine_menu_db = CantineMenuDB(db)
...
async def delete_cantine_menu_item(item_id: str, db=Depends(get_db)):
    cantine_menu_db = CantineMenuDB(db)
...
async def read_all_cantine_menu_items(db=Depends(get_db)):
    cantine_menu_db = CantineMenuDB(db)
...
async def create_hostel_menu_item(item: HostelMenuItem, db=Depends(get_db)):
    hostel_menu_db = HostelMenuDB(db)
...
async def update_hostel_menu_item(day: str, item_index: int, item: HostelMenuItem, db=Depends(get_db)):
    hostel_menu_db = HostelMenuDB(db)
...
async def delete_hostel_menu_item(day: str, item_index: int, db=Depends(get_db)):
    hostel_menu_db = HostelMenuDB(db)
...
async def read_all_hostel_menu_items(db=Depends(get_db)):
    hostel_menu_db = HostelMenuDB(db)
    items = await hostel_menu_db.get_all()
    return items

@app.get("/hostel_menu/{day}", response_model=list)
async def read_day_menu(day: str, db=Depends(get_db)):
    hostel_menu_db = HostelMenuDB(db)
