...
async def vote_hostel_meal(meal_type: str, will_eat: str, current_user: schemas.User = Depends(get_current_user)):
    ...
    if will_eat == "yes":
        ...
        await hostel_voting_db.insert_vote(email, meal=meal_type, willing_increment=1)
    else:
        await hostel_voting_db.insert_vote(email, meal=meal_type, not_willing_increment=1)
