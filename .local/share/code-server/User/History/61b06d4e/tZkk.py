...
async def create_cantine_menu_item(item: schemas.CantineMenuItem, ):
    cantine_menu_db = CantineMenuDB(app.state.mongodb)
    inserted_id = await cantine_menu_db.insert_item(item.name,item.meals, item.description, item.price)
...
async def update_cantine_menu_item(item_id: str, item: schemas.CantineMenuItem, ):
    cantine_menu_db = CantineMenuDB(app.state.mongodb)
    updated_count = await cantine_menu_db.edit_item(item_id, item.name,item.meals, item.description, item.price)
    if updated_count == 0:
