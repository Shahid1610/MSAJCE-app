...
from includes.database.CanteenMenuDB import CanteenMenuDB
...
@app.get("/canteen_menu")
async def today_food_menu(current_user: schemas.User = Depends(get_current_user)):
    canteen_menu_db = canteenMenuDB(app.state.mongodb)
    menu = await canteen_menu_db.get_all()
    return {"menu":menu}
...
async def vote_hostel_meal(meal_type: str, will_eat: str, current_user: schemas.User = Depends(get_current_user)):
    ...
    if will_eat == 'yes':
        ...
        await today_data_db.increment_willing_not_willing(meal=meal_type, willing_increment=1)
    else:
        await today_data_db.increment_willing_not_willing(meal=meal_type, not_willing_increment=1)

    # Insert vote in HostelVotingDB
    await hostel_voting_db.insert_vote(email, meal_type, will_eat)
    await user_db.update_meal_data(email=email,meal=meal_type,meal_value=True)
...
# canteenMenuDB endpoints
@app.post("/canteen_menu/", response_model=dict)
async def create_canteen_menu_item(item: schemas.canteenMenuItem, ):
    canteen_menu_db = canteenMenuDB(app.state.mongodb)
    inserted_id = await canteen_menu_db.insert_item(item.name,item.meals, item.description, item.price)
    return {"inserted_id": str(inserted_id)}

@app.put("/canteen_menu/{item_id}", response_model=dict)
async def update_canteen_menu_item(item_id: str, item: schemas.canteenMenuItem, ):
    canteen_menu_db = canteenMenuDB(app.state.mongodb)
    updated_count = await canteen_menu_db.edit_item(item_id, item.name,item.meals, item.description, item.price)
    if updated_count == 0:
        raise HTTPException(status_code=404, detail="Item not found or no update needed")
    return {"message": "Item updated"}

@app.delete("/canteen_menu/{item_id}", response_model=dict)
async def delete_canteen_menu_item(item_id: str, ):
    canteen_menu_db = canteenMenuDB(app.state.mongodb)
    deleted_count = await canteen_menu_db.delete_item(item_id)
    if deleted_count == 0:
        raise HTTPException(status_code=404, detail="Item not found")
    return {"message": "Item deleted"}

@app.get("/canteen_menu/", response_model=list)
async def read_all_canteen_menu_items():
    canteen_menu_db = canteenMenuDB(app.state.mongodb)
    items = await canteen_menu_db.get_all()
    return items
...
async def create_hostel_menu_item(item: schemas.HostelMenuItem, ):
    ...
    inserted_id = await hostel_menu_db.insert_item(item.day, item.meals,item.name, item.description)
    return {"inserted_id": str(inserted_id)}
...
async def update_hostel_menu_item(day: str, item_index: int, item: schemas.HostelMenuItem, ):
    hostel_menu_db = HostelMenuDB(app.state.mongodb)
    updated_count = await hostel_menu_db.edit_item(day, item_index, item.meals,item.name, item.description)
    if updated_count == 0:
        raise HTTPException(status_code=404, detail="Item not found or no update needed")
    return {"message": "Item updated"}
