...
class MetaData:
    ...
    async def insert_data(self, key, email, meal_type, will_eat, will_not_eat):
...
async def main():
    ...
    DB_NAME = "mydatabase"
    COLLECTION_NAME = "meal_counts"

    db = AsyncMealDatabase(MONGO_URI, DB_NAME, COLLECTION_NAME)

    # Update meal count
    modified_count = await db.update_meal_count(
        data_type="meal_count",
        time_of_day="morning",  # or "noon" or "night"
        total_will_eat_delta=5,  # Amount to increment/decrement
        total_will_not_eat_delta=2  # Amount to increment/decrement
    )
